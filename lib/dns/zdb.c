...
#define ZDB_MAGIC ISC_MAGIC('Z', 'B', 'D', '4')
...
#define VALID_RBTDB(rbtdb) \
	((rbtdb) != NULL && (rbtdb)->common.impmagic == ZDB_MAGIC)
...
struct dns_rbtdb {
	...
	/*
	 * Heaps.  These are used for zone resigning in a zone DB.
	 * hmctx is the memory context to use for the heap.
	 */
	isc_mem_t *hmctx;
	isc_heap_t **heaps;
...
static void
set_ttl(rdatasetheader_t *header, dns_ttl_t newttl) {
...
adjust_quantum(unsigned int old, isc_time_t *start) {
	...
	if (nodes != old) {
		isc_log_write(dns_lctx, DNS_LOGCATEGORY_DATABASE,
			      DNS_LOGMODULE_RBTDB, ISC_LOG_DEBUG(1),
...
new_rdataset(dns_rbtdb_t *rbtdb, isc_mem_t *mctx) {
	rdatasetheader_t *h = isc_mem_get(mctx, sizeof(*h));
...
delete_node(dns_rbtdb_t *rbtdb, dns_rbtnode_t *node) {
	...
	case DNS_RBT_NSEC_HAS_NSEC:
		...
		if (result != ISC_R_SUCCESS) {
			isc_log_write(dns_lctx, DNS_LOGCATEGORY_DATABASE,
				      DNS_LOGMODULE_RBTDB, ISC_LOG_WARNING,
				      "delete_node: "
	...
	for (;;) {
		...
		*nodep = NULL;
		...
		if (result != DNS_R_PARTIALMATCH && result != ISC_R_NOTFOUND) {
			isc_log_write(dns_lctx, DNS_LOGCATEGORY_DATABASE,
				      DNS_LOGMODULE_RBTDB, ISC_LOG_ERROR,
...
expirenode(dns_db_t *db, dns_dbnode_t *node, isc_stdtime_t now) {
	...
	isc_logmodule_t *module = DNS_LOGMODULE_RBTDB;
...
add32(dns_rbtdb_t *rbtdb, dns_rbtnode_t *rbtnode, const dns_name_t *nodename,
	...
	if (rbtversion == NULL && !newheader_nx) {
		...
		for (topheader = rbtnode->data; topheader != NULL;
			...
			if (newheader->type == sigtype &&
			    topheader->type == RBTDB_RDATATYPE_VALUE(0, covers))
			{
				break;
			}
		}
		if (topheader != NULL && EXISTS(topheader) &&
			...
			if (trust < topheader->trust) {
				...
				if (addedrdataset != NULL) {
					bind_rdataset(rbtdb, rbtnode, topheader,
						      now, isc_rwlocktype_write,
						      addedrdataset);
				}
...
addrdataset(dns_db_t *db, dns_dbnode_t *node, dns_dbversion_t *version,
	...
	/*
	 * If we're adding a delegation type (NS or DNAME),
	 ...
	 * tree, hold an exclusive lock on the tree.
	 */
...
loading_addrdataset(void *arg, const dns_name_t *name,
	...
	newheader = (rdatasetheader_t *)region.base;
	init_rdataset(rbtdb, newheader);
	set_ttl(newheader, rdataset->ttl + loadctx->now); /* XXX overflow
							   * check */
	newheader->type = RBTDB_RDATATYPE_VALUE(rdataset->type,
						rdataset->covers);
	atomic_init(&newheader->attributes, 0);
...
isc_result_t
dns_zdb_create(isc_mem_t *mctx, const dns_name_t *origin, dns_dbtype_t type,
	...
	rbtdb->nsec3_origin_node->locknum = rbtdb->nsec3_origin_node->hashval %
					    rbtdb->node_lock_count;
	...
	rbtdb->common.impmagic = ZDB_MAGIC;
...
rdatasetiter_first(dns_rdatasetiter_t *iterator) {
	...
	rbtdb_serial_t serial = rbtversion->serial;
